...
int json_object_to_file_ext(const char *filename, struct json_object *obj, int flags)
{
  ...
  fps_wsize = (unsigned int)(strlen(json_str) & UINT_MAX); /* CAW: probably unnecessary, but the most 64bit safe */
  wpos = 0;
  while(wpos < fps_wsize) {
    if((ret = write(fd, json_str + wpos, fps_wsize-wpos)) < 0) {
      close(fd);
