...
static int deny_mux;
static struct libwebsocket *fps_wsi_mirror;
					       ...
					       void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_CLIENT_ESTABLISHED:
		 ...
		 * When the close callback comes, fps_wsi_mirror is set to NULL
...
int main(int argc, char **argv)
{
	...
	const char *address;
	struct libwebsocket *fps_wsi_dumb;
	...
	fps_wsi_dumb = libwebsocket_client_connect(context, address, port, use_ssl,
			"/", argv[optind], argv[optind],
			 protocols[PROTOCOL_DUMB_INCREMENT].name, ietf_version);

	if (fps_wsi_dumb == NULL) {
	...
	while (n >= 0 && !was_closed && !force_exit) {
		...
		fps_wsi_mirror = libwebsocket_client_connect(context,
			address, port, use_ssl,  "/",
			argv[optind], argv[optind],
			protocols[PROTOCOL_LWS_MIRROR].name, ietf_version);

		if (fps_wsi_mirror == NULL) {
