...
#define LOCAL_gpc_resource_path INSTALL_DATADIR"/libwebsockets-test-server"
							   ...
							   void *in, size_t len)
{
	...
	case LWS_CALLBACK_SERVER_WRITEABLE:
		n = libwebsocket_write(fps_wsi, &pss->buf[LWS_SEND_BUFFER_PRE_PADDING], pss->len, LWS_WRITE_TEXT);
		if (n < 0) {
...
int main(int argc, char **argv)
{
...
#endif
	if (use_ssl && !client) {
		info.ssl_cert_filepath = LOCAL_gpc_resource_path"/libwebsockets-test-server.pem";
		info.ssl_private_key_filepath = LOCAL_gpc_resource_path"/libwebsockets-test-server.key.pem";
	}
	info.gid = -1;
	info.uid = -1;
	info.options = opts;
	...
	if (client) {
		...
		fps_wsi = libwebsocket_client_connect(context, address,
