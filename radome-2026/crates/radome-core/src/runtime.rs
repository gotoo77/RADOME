...
use crate::{Capability, Client, Event, Experience, MatchResult, SystemCapabilities};
...
impl Runtime {
    pub fn new(system: SystemCapabilities) -> Self { Self { clients: BTreeMap::new(), system } }
    pub fn register_client(&mut self, client: Client) { self.clients.insert(client.id.clone(), client); }
    pub fn unregister_client(&mut self, client_id: &str) -> Option<Client> { self.clients.remove(client_id) }
    pub fn evaluate(&self, experience: &Experience, client_id: &str) -> Option<MatchResult> {
        self.clients.get(client_id).map(|client| experience.evaluate(&self.system, client))
    }
    pub fn publish_for_experience(&self, experience: &Experience, event: Event) -> Vec<Delivery> {
        self.clients.values().filter(|client| experience.evaluate(&self.system, client).available)
            .map(|client| Delivery { client_id: client.id.clone(), event: event.clone() }).collect()
    }
...
mod tests {
    ...
    fn runtime_registers_and_unregisters_clients() {
        ...
        assert_eq!(runtime.client_count(), 1); assert!(runtime.unregister_client("dashboard").is_some()); assert_eq!(runtime.client_count(), 0);
    }
    ...
    fn runtime_distributes_an_event_only_to_eligible_clients() {
        ...
        assert_eq!(deliveries, vec![Delivery { client_id: "console".into(), event: event.clone() }, Delivery { client_id: "dashboard".into(), event }]);
    }
    ...
    #[test]
    fn command_capability_must_have_been_announced_by_the_client() {
        let mut runtime = Runtime::new(SystemCapabilities::default());
        runtime.register_client(Client::new("console", role("center-console"), [cap("display"), cap("media.control")]));
        runtime.register_client(Client::new("cluster", role("driver-display"), [cap("display")]));
        assert!(runtime.client_can("console", &cap("media.control")));
        assert!(!runtime.client_can("cluster", &cap("media.control")));
        assert!(!runtime.client_can("unknown", &cap("media.control")));
    }
}
