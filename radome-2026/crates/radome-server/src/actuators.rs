...
impl ActuatorState for ClimateState { fn event_data(&self)->Value{json!({"temperature_c":self.temperature_c})} }
...
impl ActuatorState for MediaState { fn event_data(&self)->Value{json!({"playback":match self.playback{PlaybackState::Playing=>"playing",PlaybackState::Paused=>"paused"},"volume":self.volume,"track_index":self.track_index})} }
...
#[test]fn climate_state_defines_its_event_representation(){assert_eq!(ClimateState{temperature_c:21.5}.event_data(),json!({"temperature_c":21.5}));}
...
#[test]fn media_state_defines_its_event_representation(){assert_eq!(MediaState{playback:PlaybackState::Playing,volume:42,track_index:3}.event_data(),json!({"playback":"playing","volume":42,"track_index":3}));}
