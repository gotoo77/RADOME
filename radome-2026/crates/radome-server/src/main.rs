...
fn new_climate_actuator() -> SharedClimateActuator { Arc::new(DemoClimateActuator::new()) }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let listener = TcpListener::bind(&addr).await?; let runtime = new_runtime(); let hub = new_hub(); let climate_actuator = new_climate_actuator();
    start_telemetry_source(Arc::clone(&runtime), Arc::clone(&hub))?;
    println!("RADOME server listening on ws://{addr}"); serve(listener, runtime, hub, climate_actuator).await
}
...
async fn serve(listener: TcpListener, runtime: SharedRuntime, hub: SharedHub, climate_actuator: SharedClimateActuator) -> Result<(), Box<dyn std::error::Error>> { loop { let (stream, peer) = listener.accept().await?; let runtime = Arc::clone(&runtime); let hub = Arc::clone(&hub); let climate_actuator = Arc::clone(&climate_actuator); tokio::spawn(async move { if let Err(error) = handle_connection(stream, runtime, hub, climate_actuator).await { eprintln!("connection {peer} closed with error: {error}"); } }); } }
