...
use radome_core::{
    Capability, Client, Envelope, MessageType, ProtocolError, ProtocolErrorCode,
    ProtocolErrorPayload, Role, PROTOCOL_VERSION,
};
        ...
        match message {
            Message::Text(text) => {
                let responses = match Envelope::decode_json(text.as_ref()) {
                    ...
                    Err(ProtocolError::InvalidJson(error)) => vec![standalone_error(
                        ProtocolErrorPayload::with_detail(
                            ProtocolErrorCode::InvalidEnvelope,
                            error.to_string(),
                        ),
                    )],
                    Err(ProtocolError::UnsupportedVersion(version)) => vec![standalone_error(
                        ProtocolErrorPayload::with_detail(
                            ProtocolErrorCode::UnsupportedVersion,
                            format!("received {version}, expected {PROTOCOL_VERSION}"),
                        ),
                    )],
                };
...
fn handle_envelope(
    ...
    let response = match incoming.message_type {
        ...
        MessageType::Hello => error_for(&incoming, ProtocolErrorCode::SessionAlreadyEstablished),
        MessageType::CapabilityAnnounce if !session.is_established() => {
            error_for(&incoming, ProtocolErrorCode::HelloRequired)
        }
        MessageType::CapabilityAnnounce => {
            handle_capability_announce(session, runtime, hub, outbound, incoming)
        }
        MessageType::DiscoveryRequest if !session.is_established() => {
            error_for(&incoming, ProtocolErrorCode::HelloRequired)
        }
        MessageType::DiscoveryRequest => handle_discovery(session, incoming),
        MessageType::StateSnapshotRequest if !session.is_established() => {
            error_for(&incoming, ProtocolErrorCode::HelloRequired)
        }
        MessageType::StateSnapshotRequest => handle_state_snapshot(session, executor, incoming),
        MessageType::Command if !session.is_established() => {
            error_for(&incoming, ProtocolErrorCode::HelloRequired)
        }
        MessageType::Command => return handle_command(session, runtime, executor, incoming),
        _ if !session.is_established() => error_for(&incoming, ProtocolErrorCode::HelloRequired),
        _ => error_for(&incoming, ProtocolErrorCode::UnsupportedMessageType),
    };
    vec![response]
}

fn handle_discovery(session: &ConnectionSession, incoming: Envelope) -> Envelope {
    if incoming.session_id.as_deref() != session.id.as_deref() {
        return error_for(&incoming, ProtocolErrorCode::InvalidSession);
    }
    let commands = commands::all()
        ...
        .map(|command| {
            json!({
                "name": command.name,
                "required_capability": command.required_capability.name(),
                "event_name": command.event_name
            })
        })
        .collect::<Vec<_>>();
    let mut capabilities = commands
        .iter()
        .filter_map(|command| command["required_capability"].as_str())
        .collect::<Vec<_>>();
    capabilities.sort_unstable();
    capabilities.dedup();
    Envelope::new(
        server_id("discovery"),
        MessageType::DiscoveryResult,
        json!({"capabilities": capabilities, "commands": commands}),
    )
...
fn handle_state_snapshot(
    ...
    Envelope::new(
        server_id("state-snapshot"),
        MessageType::StateSnapshot,
        executor.state_snapshot(),
    )
    .correlated_to(incoming.id)
    .in_session(session.id.clone().expect("established session"))
}
...
fn handle_command(
    ...
    if incoming.session_id.as_deref() != session.id.as_deref() {
        return vec![error_for(&incoming, ProtocolErrorCode::InvalidSession)];
    }
    if !session.registered {
        return vec![error_for(
            &incoming,
            ProtocolErrorCode::CapabilityAnnounceRequired,
        )];
    }
    if let Some(cached) = session.command_cache.get(&incoming.id) {
        if cached.payload == incoming.payload {
            return cached.responses.clone();
        }
    ...
    let responses = if let Some(name) = incoming.payload.get("name").and_then(Value::as_str) {
        ...
        match executor.execute(name, data, |capability| {
            ...
            Err(CommandExecutionError::InvalidPayload { code, detail }) => vec![
        ...
        vec![error_for(&incoming, ProtocolErrorCode::MissingCommandName)]
    };

    session.command_cache.insert(
        incoming.id.clone(),
        CachedCommand {
...
fn handle_hello(session: &mut ConnectionSession, incoming: Envelope) -> Envelope {
    let Some(client_id) = incoming.payload.get("client_id").and_then(Value::as_str) else {
        return error_for(&incoming, ProtocolErrorCode::MissingClientId);
    };
    let session_id = session.establish(client_id.to_owned());
    Envelope::new(
        "server-hello",
        MessageType::Hello,
        json!({"server": "radome-server", "protocol_version": PROTOCOL_VERSION}),
    )
    .correlated_to(incoming.id)
...
fn handle_capability_announce(
    ...
    let Some(role) = incoming.payload.get("role").and_then(Value::as_str) else {
        return error_for(&incoming, ProtocolErrorCode::MissingRole);
    };
...
fn error_payload(payload: ProtocolErrorPayload) -> Value {
    serde_json::to_value(payload).expect("protocol error payload serializes")
}

fn standalone_error(payload: ProtocolErrorPayload) -> Envelope {
    Envelope::new(server_id("error"), MessageType::Error, error_payload(payload))
}
...
mod tests {
    use super::*;
    use crate::actuators::{DemoClimateActuator, DemoMediaActuator};
    ...
    fn runtime() -> SharedRuntime {
        Arc::new(std::sync::Mutex::new(Runtime::new(SystemCapabilities::new([
            Capability::new("vehicle.telemetry"),
        ]))))
    }

    fn hub() -> SharedHub {
        Arc::new(std::sync::Mutex::new(ConnectionHub::default()))
    }

    fn climate() -> SharedClimateActuator {
        Arc::new(DemoClimateActuator::new())
    }

    fn media() -> SharedMediaActuator {
    ...
    async fn receive<S>(socket: &mut tokio_tungstenite::WebSocketStream<S>) -> Envelope
    where
        S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
    {
    ...
    async fn hello(
        ...
        send(
            ...
            Envelope::new(
                "hello-test",
                MessageType::Hello,
                json!({"client_id": "integration-console"}),
            ),
        )
        .await;
        receive(socket).await.session_id.unwrap()
    }
    ...
    async fn establish(
        ...
        let session_id = hello(socket).await;
        send(
            socket,
            Envelope::new(
                "caps-test",
    ...
    #[tokio::test]
    async fn protocol_errors_use_stable_code_contract() {
        let (mut socket, server) = fixture().await;
        send(
            &mut socket,
            Envelope::new(
                "discover-before-hello",
        ...
        assert_eq!(error.correlation_id.as_deref(), Some("discover-before-hello"));
        assert_eq!(error.payload, json!({"code": "hello_required"}));
        socket.close(None).await.unwrap();
        server.await.unwrap();
    }

    #[tokio::test]
    async fn malformed_wire_message_returns_stable_invalid_envelope_code() {
        let (mut socket, server) = fixture().await;
        socket
        ...
        let error = receive(&mut socket).await;
        assert_eq!(error.message_type, MessageType::Error);
        assert_eq!(error.payload["code"], "invalid_envelope");
    ...
    #[tokio::test]
    async fn client_bootstraps_from_discovery_without_hard_coded_server_capabilities() {
        let (mut socket, server) = fixture().await;
        ...
        .await;
        let discovery = receive(&mut socket).await;
        assert_eq!(discovery.message_type, MessageType::DiscoveryResult);
        let offered = discovery.payload["capabilities"].as_array().unwrap();
        let supported_by_client = ["media.control"];
        let selected = offered
            .iter()
            .filter_map(Value::as_str)
            .filter(|capability| supported_by_client.contains(capability))
            .collect::<Vec<_>>();
        assert_eq!(selected, vec!["media.control"]);
        send(
            &mut socket,
            Envelope::new(
                "bootstrap-capabilities",
                MessageType::CapabilityAnnounce,
                json!({"role": "center-console", "capabilities": selected}),
            )
            .in_session(session_id.clone()),
        )
        .await;
        ...
        assert_eq!(
            ...
            json!({"playback": "paused", "volume": 50, "track_index": 0})
        );
        send(
            &mut socket,
            Envelope::new(
    ...
    #[tokio::test]
    async fn snapshot_after_commands_matches_the_last_emitted_states() {
        let (mut socket, server) = fixture().await;
        let session_id = establish(
            &mut socket,
            json!(["climate.control", "media.control"]),
        )
        .await;
        send(
            &mut socket,
            Envelope::new(
                "cmd-climate-sync",
                MessageType::Command,
                json!({"name": "climate.set_temperature", "data": {"temperature_c": 23.0}}),
            )
            .in_session(session_id.clone()),
        )
        .await;
        assert_eq!(receive(&mut socket).await.payload["outcome"], "succeeded");
        let climate_event = receive(&mut socket).await;
        assert_eq!(
            climate_event.payload["data"],
            json!({"temperature_c": 23.0})
        );
        send(
            &mut socket,
            Envelope::new(
        ...
        .await;
        assert_eq!(receive(&mut socket).await.payload["outcome"], "succeeded");
        let media_event = receive(&mut socket).await;
        assert_eq!(
            media_event.payload["data"],
            json!({"playback": "paused", "volume": 72, "track_index": 0})
        );
        send(
            &mut socket,
            Envelope::new(
        ...
        .await;
        let snapshot = receive(&mut socket).await;
        assert_eq!(snapshot.payload["climate"], climate_event.payload["data"]);
        assert_eq!(snapshot.payload["media"], media_event.payload["data"]);
        socket.close(None).await.unwrap();
        server.await.unwrap();
    }

    #[tokio::test]
    async fn websocket_client_can_request_state_snapshot_after_hello() {
        let (mut socket, server) = fixture().await;
        let session_id = hello(&mut socket).await;
        send(
            &mut socket,
        ...
        .await;
        let result = receive(&mut socket).await;
        assert_eq!(result.message_type, MessageType::StateSnapshot);
        assert_eq!(result.correlation_id.as_deref(), Some("snapshot-test"));
        assert_eq!(result.session_id.as_deref(), Some(session_id.as_str()));
        assert_eq!(
            result.payload,
            json!({
                "climate": {"temperature_c": 20.0},
    ...
    #[tokio::test]
    async fn websocket_client_can_discover_commands_after_hello() {
        let (mut socket, server) = fixture().await;
        let session_id = hello(&mut socket).await;
        send(
            &mut socket,
            Envelope::new("discover-test", MessageType::DiscoveryRequest, json!({}))
                .in_session(session_id.clone()),
        )
        .await;
        let result = receive(&mut socket).await;
        assert_eq!(result.message_type, MessageType::DiscoveryResult);
        assert_eq!(result.correlation_id.as_deref(), Some("discover-test"));
        assert_eq!(result.session_id.as_deref(), Some(session_id.as_str()));
        assert_eq!(
            result.payload["capabilities"],
            json!(["climate.control", "media.control"])
        );
        assert_eq!(result.payload["commands"].as_array().unwrap().len(), 9);
        assert!(result.payload["commands"]
            .as_array()
            .unwrap()
            .iter()
            .any(|command| {
                command["name"] == "media.play"
                    && command["required_capability"] == "media.control"
            }));
        socket.close(None).await.unwrap();
        server.await.unwrap();
    }
    ...
    async fn websocket_client_can_execute_climate_command_end_to_end() {
        let (mut socket, server) = fixture().await;
        let session_id = establish(&mut socket, json!(["climate.control"])).await;
        send(
            &mut socket,
            Envelope::new(
                "cmd-climate",
                MessageType::Command,
                json!({"name": "climate.set_temperature", "data": {"temperature_c": 21.5}}),
            )
            .in_session(session_id),
        )
        .await;
        assert_eq!(receive(&mut socket).await.payload["outcome"], "succeeded");
        assert_eq!(
            receive(&mut socket).await.payload["name"],
            "climate.temperature_changed"
        );
        socket.close(None).await.unwrap();
        server.await.unwrap();
    }
    ...
    #[tokio::test]
    async fn websocket_client_can_execute_media_command_end_to_end() {
        let (mut socket, server) = fixture().await;
        ...
        assert_eq!(result.payload["outcome"], "succeeded");
        assert_eq!(result.session_id.as_deref(), Some(session_id.as_str()));
        assert_eq!(result.correlation_id.as_deref(), Some("cmd-media"));
        assert_eq!(event.session_id.as_deref(), Some(session_id.as_str()));
        assert_eq!(event.correlation_id.as_deref(), Some("cmd-media"));
        assert_eq!(event.payload["name"], "media.next_track_requested");
        assert_eq!(
            event.payload["data"],
            json!({"playback": "paused", "volume": 50, "track_index": 1})
        );
        socket.close(None).await.unwrap();
        server.await.unwrap();
    }
    ...
    #[tokio::test]
    async fn duplicate_command_id_replays_without_second_actuation_and_conflicts_are_rejected() {
        let (mut socket, server) = fixture().await;
        let session_id = establish(&mut socket, json!(["media.control"])).await;
        ...
        .in_session(session_id.clone());
        send(&mut socket, command.clone()).await;
        ...
        let replay_result = receive(&mut socket).await;
        let replay_event = receive(&mut socket).await;
        assert_eq!(replay_result, first_result);
        assert_eq!(replay_event, first_event);
        send(
            &mut socket,
            Envelope::new(
                "snapshot-idempotent",
        ...
        assert_eq!(conflict.payload, json!({"code": "message_id_conflict"}));
        assert_eq!(conflict.correlation_id.as_deref(), Some("cmd-idempotent"));
        send(
            &mut socket,
            Envelope::new(
                "snapshot-after-conflict",
                MessageType::StateSnapshotRequest,
                json!({}),
            )
            .in_session(session_id),
    ...
    async fn websocket_denied_command_emits_no_domain_event() {
        let (mut socket, server) = fixture().await;
        let session_id = establish(&mut socket, json!([])).await;
        send(
            &mut socket,
            Envelope::new(
                "cmd-denied",
        ...
        .await;
        assert_eq!(
            receive(&mut socket).await.payload["data"],
            "capability_denied"
        );
