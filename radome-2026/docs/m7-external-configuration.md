...
Les variables historiques restent donc utilisables pour surcharger ponctuellement un déploiement :
...
Le smoke test live `clients/sdk/radome-live.e2e.mjs` ne démarre plus le serveur via `RADOME_ADDR` et `RADOME_TELEMETRY_SOURCE`. Il crée un vrai fichier temporaire `server.json`, définit uniquement `RADOME_CONFIG`, lance le binaire Rust, puis exerce la boucle complète serveur ↔ SDK.
...
La prochaine tranche recommandée est le `tracing` structuré, maintenant que les paramètres de démarrage sont centralisés et peuvent fournir un contexte d'exploitation stable.
